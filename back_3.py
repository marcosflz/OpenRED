...
class EngineCADBuilder_ConventionalNozzle:
    ...
    def get_sketch(self, user_settings, sketch='Engine'):
        ...
        xCover = np.array([
            -self.engineData["inputs"]["Lc"] + xConvCon,#1
            -self.engineData["inputs"]["Lc"] + xConvCon,#2
            -self.engineData["inputs"]["Lc"] + xConvCon + user_settings["bolt_OffSet"],#3
            -self.engineData["inputs"]["Lc"] + xConvCon + user_settings["bolt_OffSet"],#4
            -self.engineData["inputs"]["Lc"] + xConvCon - user_settings["cover_t"],#5
            -self.engineData["inputs"]["Lc"] + xConvCon - user_settings["cover_t"],#-5
            #-self.engineData["inputs"]["Lc"] + xConvCon,#
            -self.engineData["inputs"]["Lc"] + xConvCon + user_settings["bolt_OffSet"],#-4
            -self.engineData["inputs"]["Lc"] + xConvCon + user_settings["bolt_OffSet"],#-3
        ...
        yCover = np.array([
            0,#user_settings["r_elect"] + user_settings["d_elect"] / 2,
            yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"] * user_settings["hBoltFactor"],
            yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"] * user_settings["hBoltFactor"],
            yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"],
            yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"],
            ...
            -(yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"] * user_settings["hBoltFactor"]),
            0
            #user_settings["r_elect"] + user_settings["d_elect"] / 2,
            #user_settings["r_elect"] + user_settings["d_elect"] / 2
        ])

        #xCoverMiddle = np.array([
        #    -self.engineData["inputs"]["Lc"] + xConvCon,
        #    -self.engineData["inputs"]["Lc"] + xConvCon,
        ...
        xProp = self.propellant_X + xConvCon
        yProp = self.propellant_Y


        R_out = yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"]
        R_in = yConvCon + user_settings["t_cartridge"] + user_settings["wall_t"] * user_settings["hBoltFactor"]
        R_holes = user_settings["r_elect"]
        R_elect = user_settings["d_elect"] / 2
        
        R_nOut = self.rExit + user_settings["wall_t"]
        R_nIn = self.rExit
        R_nThr = self.rThroat
        #R_prop = np.min(self.propellant_Y) # Esto habria que cambiarlo por algo que dibujara la geometria real de cada propelente 
        # Eso va a haber que importarlo a traves del archivo del motor osea que se construya en la clase del grano


        

        if sketch == 'Engine':
            x = np.concatenate((xNozzle, xConvThr, [xConvCon], xCase, xWallPoints, xNozzleWall))
            y = np.concatenate((yNozzle, yConvThr, [yConvCon], yCase, yWallPoints, yNozzleWall))
            data = x, y
        if sketch == 'Cover':
            x = xCover
            y = yCover
            data = x, y
        #if sketch == 'CoverMiddle':
        #    x = xCoverMiddle
        #    y = yCoverMiddle
        #    data = x, y
        if sketch == 'Cartridge':
        ...
        if sketch == 'CoverFront':
            data = R_out, R_in, R_holes, R_elect 
        if sketch == 'NozzleFront':
            data = R_out, R_nOut, R_nIn, R_nThr
    ...
    def plot_Engine(self, user_settings):
        ...
        if user_settings["on_Engine"]:
            ...
            ax.fill(xEng, yEng, color='lightgray', alpha=1, edgecolor='black', lw=2)
            ax.fill(xEng, -yEng, color='lightgray', alpha=1, edgecolor='black', lw=2)

        if user_settings["on_Cover"]:
            xCov, yCov = self.get_sketch(user_settings, sketch='Cover')
            #xCovM, yCovM = self.get_sketch(user_settings, sketch='CoverMiddle')

            ax.fill(xCov, yCov, color='lightgray', alpha=1, edgecolor='black', lw=2)
            #ax.fill(xCovM, yCovM, color='lightgray', alpha=1, edgecolor='black', lw=2)
        ...
        if user_settings["on_Cartridge"]:
            ...
            ax.fill(xCart, yCart, color='grey', alpha=1, edgecolor='black', lw=2)
            ax.fill(xCart, -yCart, color='grey', alpha=1, edgecolor='black', lw=2)
    ...
    def plot_CoverFront(self, user_settings):
        ...
        R_out, R_in, R_holes, R_elect = self.get_sketch(user_settings, sketch='CoverFront')
        ...
        circle_in = Circle((0, 0), R_in, edgecolor='k', facecolor='lightgray', fill=True, linewidth=2)
        
        # Dibujar los electrodos en la parte superior e inferior
        elect1 = Circle((0, R_holes), R_elect, edgecolor='k', facecolor='white', fill=True, linewidth=1)
        elect2 = Circle((0, -R_holes), R_elect, edgecolor='k', facecolor='white', fill=True, linewidth=1)
        ...
        max_radius = R_out + R_elect  # Radio máximo que queremos visualizar
    ...
    def plot_NozzleFront(self, user_settings):
        ...
        # Dibujar los electrodos en la parte superior e inferior
        nozzle1 = Circle((0, 0), R_nIn, edgecolor='k', facecolor='grey', fill=True, linewidth=2)
