...
class PropellantDesignModule:
    ...
    def run_simulation(self) -> None:
        ...
        # Configure grain configuration class
        grainConfigClass = self.selected_grain_config(self.init_specs, self.specInputs["R2"], self.specInputs["dh"])
        init_geo = grainConfigClass.getPhi()  # Get the initial geometry
        self.propCAD = grainConfigClass.sketchCAD()  # Generate cad design

        # Initialize simulation
        self.simulation = PropellantRegresionLSM(
            textbox=0,  # Text interface (to be defined)
            dt=self.specInputs["dt"],  # Time step
            dh=self.specInputs["dh"],  # Space step
            maxIters=self.specInputs["It"],  # Maximum number of iterations
            phi0=init_geo,  # Initial geometry
            init_data=self.specInputs,  # Initial entry data
            workingPrecision=8,  # Working precision
            image_resolution=30  # Image resolution
        )

        # Execute simulation and store results
        self.results = self.simulation.run(self.console_box)  # Save the results in the console box


    def check_simulation_thread(self) -> None:
        """Check if the simulation thread is still running and update results when finished."""
        if self.simulation_thread.is_alive():  # Verify if the simulation thread is still active
            # If the thread continues to run, verify again after 100 ms
            self.main_frame.after(100, self.check_simulation_thread)
        else:
            # Once the thread is over, update the results
